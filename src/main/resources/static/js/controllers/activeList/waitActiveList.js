...
app.controller('waitActiveListController',['$scope', '$http','$modal','toaster',function ($scope,$http,$modal,toaster) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            { field: 'remove', displayName: '操作类型', width:'160px', cellTemplate: '<a ng-click="operation(row.entity)" title="操作" class="btn btn-default m-l-xs" style="margin-top: 2px;margin-left: 5px"><i class="fa fa-gear"></i></a>' +
            '<a ng-click="seeRowIndex(row.entity)" title="明细" class="btn btn-default m-l-xs" style="margin-top: 2px"><i class="fa fa-info-circle"></i></a>'+
            ...
            { field: 'activeFailCount', displayName: '激活失败次数', width:'120px' },
            { field: 'waitActiveTime', displayName: '待激活开始日期', width:'150px', cellFilter: "date: 'yyyy-MM-dd HH:mm:ss'" },
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        var url  = '/active/waitActive?page=' + page + '&size=' + pageSize + '&simCode=' + $scope.simCode + '&vehicleIdentifyNum='
            + $scope.vehicleIdentifyNum + '&applyNum=' + $scope.applyNum + '&userIdentityNumber=' + $scope.userIdentityNumber;
        var waitActiveTime = $("#beginTime").val();
        if (waitActiveTime != "" && waitActiveTime != null) {
            url += '&waitActiveTime=' + waitActiveTime;
        }
    ...
    // 操作处理
    $scope.operation = function (entity) {

        var rtn = $modal.open({
            templateUrl: 'tpl/activeList/active_operation.html',
            controller: 'activeOperationController',
            resolve:{
                activeDate:function () {
                    return entity;
                },
