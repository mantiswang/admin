...
app.controller('accOffReportController',['$scope', '$http', 'toaster', '$window','$filter', function ($scope, $http, toaster, $window,$filter) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs:[
            ...
            {field:'accOffStartTime', displayName:'ACC关开始时间', width: '150px', cellFilter: "date: 'yyyy-MM-dd HH:mm:ss'"},
            {field:'accOffEndTime', displayName:'ACC关结束时间', width: '150px', cellFilter: "date: 'yyyy-MM-dd HH:mm:ss'"},
            {field:'accOffDurationSize', displayName:'ACC关时长', width: '100px'},
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        ...
        var url = '/accOffReport?page=' + page + '&size=' + pageSize + '&simCode=' + $scope.simCardNum + '&vehicleIdentifyNum=' + $scope.vehicleIdentifyNum +
            '&applyNum=' + $scope.applyNum + '&startTime=' + startTime + '&endTime=' + endTime;
        $scope.$emit("BUSY");
        $http.get(url).success(function (pageData) {
            $scope.$emit("NOTBUSY");
            if (pageData.status == 'SUCCESS') {
                $scope.accOffData = pageData.data.content;
    ...
    $scope.export = function () {
        ...
        var url = 'excel/reportAccOff?simCode=' + $scope.simCardNum + '&vehicleIdentifyNum=' + $scope.vehicleIdentifyNum +
            '&applyNum=' + $scope.applyNum + '&startTime=' + startTime + '&endTime=' + endTime;
