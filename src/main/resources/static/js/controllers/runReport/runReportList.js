...
app.controller('runListController', ['$scope', '$http', '$modal', 'toaster', '$filter','$window', function ($scope, $http, $modal, toaster ,$filter,$window) {
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            { field: 'runStartTime', displayName: '行驶开始时间', width: "150px", cellFilter: "date:'yyyy-MM-dd HH:mm:ss'"},
            { field: 'runStopTime', displayName: '行驶结束时间', width: "150px", cellFilter: "date:'yyyy-MM-dd HH:mm:ss'"},
            { field: 'timeSize', displayName: '行驶时长', width: "100px"},
            { field: 'runDistanceSize', displayName: '行驶里程', width: "100px"},
            { field: 'startAddress', displayName: '行驶开始位置', width:'200px' },
            { field: 'endAddress', displayName: '行驶结束位置', width:'200px' }
    ]
    };
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        ...
        //     return;
        // }
        var url = '/runReport/getHistory?page=' + page + '&size=' + pageSize  +'&simCode=' +$scope.simCardNum +'&vehicleIdentifyNum=' +$scope.vehicleIdentifyNum +'&applyNum=' +$scope.applyNum;
    ...
    $scope.export = function () {
        ...
        var url = '/excel/reportRun?simCode=' +$scope.simCardNum +'&vehicleIdentifyNum=' +$scope.vehicleIdentifyNum +'&applyNum=' +$scope.applyNum;
        if($scope.beginTime != "" && $scope.selectType == 0){
            url+='&beginTime='+$scope.beginTime;
        }
        if($scope.endTime != "" && $scope.selectType == 0){
