...
/* Controllers */
  ...
  ;
app.controller('headerController', ['$state', '$localStorage', '$scope',  '$http','toaster',function($state, $localStorage, $scope, $http,toaster) {
  ...
  $scope.changePassword = function(){
    ...
    $http.get("sysusers/getLoginSysUser").success(function(data){
      ...
      var loginSysUser = data.data;
      //如果用户可以修改密码进入密码修改界面
      if (loginSysUser.isModifyPassWord=='1'){
        $state.go("app.changePassword");
      }else {
        $scope.pop('error', '', "您没有权限修改密码，请联系管理员");
        //alert("您没有权限修改密码，请联系管理员");
      }
    });
  }
}])
