 ...
 */
app.controller('updateAreaController', ['$scope', '$http', '$modal', 'toaster', '$state', '$interval', 'Fullscreen', 'toaster', '$filter','$location', function ($scope, $http, $modal, toaster, $state, $interval, Fullscreen, toaster, $filter,$location) {
    ...
    function init() {
        ...
        $http.get('areas/'+ $location.search()['areaId']).success(function(data){
            $scope.areaDto.lonlatstrArr = data.data.lonlatstrArr;
            maplet = new Maplet("allmap");
            maplet.centerAndZoom(new MPoint($scope.areaDto.lonlatstrArr[0]),7);
